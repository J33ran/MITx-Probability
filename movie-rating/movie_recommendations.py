...
def compute_posterior(prior, likelihood, y):
    ...
    user_likelihood = likelihood[y,]
    user_log_likelihood = np.log(user_likelihood)

    log_numerator = [log_prior] + np.sum(user_log_likelihood, axis=0)
...
def compute_movie_rating_likelihood(M):
    ...
    #likelihood
    likelihood=np.array([[np.abs(alpha/(j-i)) if i != j else (2*alpha) for j in range(M)] for i in range(M)])
...
def infer_true_movie_ratings(num_observations=-1):
    ...
    for i in range(num_movies):
        ratings = movie_data_helper.get_ratings(i)
    ...
    max_rating = np.amax(MAP_ratings)
    top_movies = [i for i in range(len(MAP_ratings)) if MAP_ratings[i] == max_rating]

    #top_10_movies = np.argsort(MAP_ratings)[-10:][::-1]
    #print(top_10_movies)
    #movies_list = [movie_data_helper.get_movie_name(i) for i in top_10_movies]
